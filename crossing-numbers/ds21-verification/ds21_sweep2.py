...
if __name__ == "__main__":
    ...
    # Revalidated in every batch, not once: the decider changed between runs,
    # and a sweep is only as good as the instrument it last checked.
    bad = 0
    for name, G, val in [("K5", nx.complete_graph(5), 1),
        ...
        ok = got == val
        bad += not ok
        print(f"   cr({name}) = {got}  expected {val}  {'OK' if ok else 'FAIL'}")
    if bad:
        sys.exit("decider failed validation; sweep not run")
    print()
        ...
        for n in (1, 2, 3, 4):
            ...
            if pred > 12:
                print(f"{name:>15} {n:>3} {G.number_of_nodes():>4} "
                      ...
                      f"out of range (predicted {pred} > 12)", flush=True)
                continue
            t0 = time.time()
            got = crk2.cr(G, cap=pred + 1, budget=600.0)
            if got is None:
                v = "not decided (time)"
            elif got == pred:
                v = "OK"
