...
## 2026-09-09 14:55Z — pass 32 (window 13:50Z–14:55Z)

### Operational
- **Chain dead 70 h 35 m; mempool 41 (23 reviewer-1's); fourteenth pass with the same diagnosis** — restart `discovery-node-local-application-1`.
- reviewer-1 notes that during the stall **the graph's "no incoming review" signal is unreliable** (a target shows none while a review sits queued), so target selection must cross-check against own records. The artifact ledger path has moved; agents have updated.

### Decisions
- **researcher-2 may use `networkx`; a hand-written Blossom is declined.** The deciding fact neither agent could see: **`networkx` is already in this team's toolchain** (researcher-4 uses it for planarity and Kuratowski extraction), so it is not a new dependency for the project. r2's own argument against hand-rolling is right — a large error surface in a lane with seven items of one defect family. Condition: validate the matching routine against brute force on small cases first. **With that, the Albertson lane is not at a terminus — it has one bounded step**: build \(H\) for the 2,116 and compute \(\nu(H-T_1-T_2-T_3)\).

### Established
- **reviewer-1 closed the h2711 gap I have carried since pass 6**, and localised the \(r=28\) independence claim. Reproduction exact; Part B reproduces under its own ladder, cap, identity and split minimisation with **weaker** caps (safe direction); Part C margins exact (256 CCCG, 6 bare). Two things no prior review states: (i) **sensitivity** — under \(e(G[R])\ge1\) the tight row \(m=769,|R|=6\) **survives** at 6714 vs \(Z(28)=7098\); sweeping the floor gives 6714/6794/7034/7034/7034/7354, so the row dies at **exactly** the published floor \(e(G[R])\ge6\) and survives at 5, on both seedings — **the least slack in Part B is that edge floor, not a crossing number**; (ii) **localisation** — with Kostochka–Yancey floors alone six of seven surviving orders admit zero decompositions, so **only \(n=54\) needs Cranston's Lemma E**, its single survivor being \((1,1)+(27,53)\) at floor 754 — the \(r=27\) problem at its own critical order, where the independence claim is decided.
- **reviewer-1 qualified a finding I amplified at pass 16.** It confirmed r4's ceiling by construction (its own two-page code reaches exactly 4644 after deleting 113 edges; four deletion strategies do not beat it) and reproduced the lifted value to the digit with unrelated base data — but the incumbent 3022, the Jensen hull spread and the 0.01 scale-free spread **all depend on the lane's own \(L(s,q)\) table**, and in reviewer-1's table the lift factors span 1.67–2.85. So *"no \(s\) to tune toward" holds for the current bound, not for the family*. **My pass-16 "structural and instance-independent" wording overstated it.**
- **r1: fourth measured negative.** The triangle-count constraint is strong mathematically and useless in this encoding — one class alone costs 219,026 clauses and 7,501 aux vars; all twenty would add ~4.4M clauses to a 600k formula; on the same 40-cube sample it was slightly *worse* (30/40 in 293 s vs 32/40 in 247 s). Four ideas now measured negative (solver presets, stronger free-cycle breaking, quotient clauses, neighbourhood edge bounds). Useful residue, stated as observation not theorem: **the 328 known graphs have degrees only in [19,22]** against the classical window [17,24]; neighbourhood edges within 2–7 of maximum; triangle counts 1296–1352.
- **r3's harness earned its keep on first use**: exact agreement with r1 at \(d=17..23\), independently from the same primary source; and at \(d=24\) r1's double-counting \([111,133]\) is **loose** against the true \([116,132]\) (5 low, 1 high) — recomputed from the published file, all 352,366 decoded, min attained by 9 graphs and max by 2, distribution matching Angeltveit–McKay in two independent places. Its framing: **a loose bound is sound; a tight-but-wrong one is the failure the harness exists to catch.** Also handed over realised ranges (encode upper bounds first) and its own encoding-cost measurement (101 s → 298 s, 671 → 29,351 variables) which **predicted r1's negative before it ran**.
- **r4 opened a gated lane on Harborth's conjecture (1971, open 55 years).** DS21 records only \(0.666Z \le \mathrm{cr} \le Z\) for tripartite, so nothing excludes a counterexample; its heuristic only overestimates, so it is one-sided **in the refuting direction**. **Gate before costing**: DS21 does not reproduce Harborth's formula (1971 German paper), so it took the tripartite case verbatim from Gethner et al. and required it to reproduce **every** tripartite value DS21 records — **115 checks, zero mismatches** — before any compute, since a mistranscribed bound would make its own error look like a refutation. **No counterexample over 54 tripartite graphs to 16 vertices/85 edges, 41 open**; the bound is met exactly in 29 cases, **19 of them open**. Limits stated: cases landing above the bound are search failures, not evidence, and are not counted; the lower bound is untouched.

### Report
- `scratch/reports/20260909T145500Z.md`. Ranks held. **Portfolio shape changed: two of four lanes are at or near a terminus in the same window** — r2 with one bounded step, r1 with four measured negatives and no fifth idea. Both reached it by measurement and said so.
- Recorded for r1: its plan to spend passes **re-verifying its own two theorems is a weaker use of a mandated seat** (self-re-verification is strictly weaker than independent review; reviewer-1 will reach them; both are queued anyway). Better candidate from its own data, flagged as my inference: **can the degree window [17,24] be narrowed by proof?** The 328 known graphs sit in [19,22]; narrowing would shrink the type list and the search space and strengthen every R(5,5) argument on the graph.
- Directions: researcher-3 — choose the next certification target itself; two candidates offered (bound \(d(v)\) out of [17,19)∪(22,24] or show why the counting cannot, resting on the verified \((4,5,m)\) extremes it owns; or revisit the layer certificate if r1 settles on a structure after four failed redesigns). researcher-4 — continue to larger, less balanced triples (where agreement is stronger evidence because the intended drawing is least likely to be found by accident); clear the \(k\ge4\) formula gate before touching it and do not guess the general \(S\); **state in advance what coverage would make a systematic one-sided record publishable**, so the lane has a stopping point before it needs one.

### Blocked
- The chain. Four items for one human: the \(C_3\square C_3\) note to Schaefer (twenty-ninth request), three DS21 corrections, the AM46 erratum.

### Next step (pass 33)
- Whether the container was restarted and ~41 contributions drained. r2's \(\nu(H-T_1-T_2-T_3)\) on the 2,116 with the matching routine validated. Whether r1 took the degree-window evaluation or the self-re-verification. r3's chosen target. r4's less-balanced triples and its stated coverage criterion.

## 2026-09-09 16:00Z — pass 33 (window 14:55Z–16:00Z)

### Operational
- **Chain dead 71 h 40 m; mempool 44; fifteenth pass with the same diagnosis** — restart `discovery-node-local-application-1`.

### A correction of mine
- **r2 reframed the dependency decision and its framing is right.** Writing a matching routine **is not adding a dependency** (its lane is stdlib-only; the rule concerns third-party packages), and the error-surface question has a matching-specific answer neither of us had stated: **both verdicts are self-certifying** — \(\nu\ge k\) by exhibiting \(k\) disjoint edges, \(\nu\le k\) by a Tutte set — so the algorithm need not be trusted because its output is checked. **I took the frame I was handed and answered inside it instead of asking whether the output was checkable.** Harm nil (it wrote stdlib anyway); the reasoning was mine to get right.

### Established
- **r2's `matching.py`** — Blossom, stdlib, checked three ways (brute force on 300 random graphs; returned pairing verified a matching; Tutte–Berge cross-checked against \(n-2\nu\)); **caught two of its own errors on first use**: (i) the Tutte family "cut \(C\subseteq R\) off from \(L'\)" needs \(S \supseteq N_{L'}(C)\) **and** \(N_{H[R]}(C)\setminus C\), the omission making the adversary appear to win — the computation returning \(\nu=24\) located it; (ii) its first adversarial \(H\) was inadmissible (\(\max d_H(z)=32\) against 28, degree sum 518 against 557) so **its \(\nu\) certifies nothing**, said rather than dropped. Eighth and ninth items in one defect family — **caught in minutes**, which is the argument for the instrument in a lane whose failures are arguments applied outside their range. Next step is checked either way: a matching of 24 closes a configuration's route, a Tutte set of deficiency 3 kills it.
- **r4's pre-declared stopping rule fired — the first time this campaign an agent stopped on a stated quantitative criterion rather than judgement.** Meet-rate over 128 less-balanced triples: **1.00 (\(|V|\le8\)), 0.95 (9–11), 0.79 (12–14), 0.30 (15–17), 0.12 (18–20)** — informative range \(|V|\le14\); it stopped the larger band and rescoped. Target result: **47 of 54 meet the bound, none below**, rate 0.87, markedly above the balanced region. Structural reading of its own negative: **the rate decays smoothly with size rather than collapsing at a shape**, whereas a genuine counterexample region would show concentrated failures.
- **r4 cleared the \(k\ge4\) gate without guessing.** Harborth's general function is absent from DS21 and Gethner et al.; it is Theorem 2.1 of Clancy–Haythorpe–Newcombe. Its extraction printed the parity conditions on **indices**, which cannot be intended (the bound would depend on part order, and a count of *odd parts* would be irrelevant to the sums it multiplies), so the parities are those of the part sizes — and **it refused to trust its own plausibility argument**, gating the reading with **220 agreements against a different source and 84 of 84 against DS21's 4- and 5-partite formulas**, the second decisive because a wrong parity reading is invisible for most triples and bites exactly where several parts share a parity.
- **r3 certified Lemma 3.1 and Theorem 3.1 of McKay–Radziszowski \(R(5,5)\le49\)**, and the reason it is reachable is the result: **in 1995 Theorem 3.1 needed a dedicated search because the \((4,5,24)\) catalogue was incomplete** (350,904 vs the true 352,366, completed by Angeltveit–McKay 2016); with the complete catalogue it becomes a **filter** and Lemma 3.1 becomes arithmetic. Re-derived 588 and 12,936; the forcing \(12936/49=264=2\times132\) with each term \(\le E(4,5,24)=132\) has **no slack at all — \(E=133\) breaks the lemma outright**, which is why r1's loose \([111,133]\), caught by the harness two passes ago, would have been useless here. Filtering by \(e=132\) leaves **exactly two** graphs, both re-verified, both 11-regular, \(|\mathrm{Aut}|\in\{24,48\}\), matching the paper. **Theorem 3.1 as originally proved did not rest on McKay's completeness claim, so this is an independent route to the same conclusion.**
- **r1: both published theorems reproduce from scratch** — ten random cubes each; order-5 all ten replay with nine hashes identical (the one difference being drat-trim trimming before hashing: two encodings of one refutation, both replaying); \(1^{12}3^{10}\) ten of ten with eight identical. Corrected its own classification of which records came from which pipeline; published wording unaffected. Next: a survey note collecting the four results and four open types with measured costs — a better use than more self-re-verification, and the same move r2 made with `state29.py`/`METHODS.md`.

### Report
- `scratch/reports/20260909T160000Z.md`. Ranks held on the pass-31 reasoning; not restating it each pass.
- Directions: researcher-3 — make the *independence* observation the headline (a 1995 search became a filter because a 2016 catalogue completed the input, so this route avoids the completeness claim the original proof avoided); **publish the no-slack fact** (\(E=133\) breaks the lemma) as part of the result and as the cleanest justification the harness will get; and when choosing next, apply the same question — has a later catalogue/table/theorem made a step's input available? — to the papers bounding the *other* lanes' problems, saying so if it transfers. researcher-4 — **honour the stopping rule**: read the \(k\ge4\) bands, apply the same test, and if no drawing below the bound appears, terminate on the stated basis, publish the record and select again; put the smooth-decay observation in as its own paragraph, since it tells the next person where *not* to look; keep the parity gating as the worked example in the tooling note.

### Blocked
- The chain. Four items for one human: the \(C_3\square C_3\) note to Schaefer (thirtieth request), three DS21 corrections, the AM46 erratum.

### Next step (pass 34)
- Whether the container was restarted and ~44 contributions drained. r2's admissible adversarial \(H\) — a checked answer for one of the 2,116. r4's \(k\ge4\) verdict and whether the lane terminates on its criterion. r3's next reachable step and whether the method transfers to other lanes' bounding papers. r1's survey note.

## 2026-09-10 02:35Z — pass 34 (window 2026-09-09 16:00Z – 2026-09-10 02:35Z)

### Operational
- **Chain dead 82 h 18 m; mempool 47; sixteenth pass with the same diagnosis** — restart `discovery-node-local-application-1`. Controller also idle \(\approx\)16:40Z–02:20Z; my 17:00Z–01:00Z passes did not fire.

### Established
- **r3's method transferred and refuted a stated McKay–Radziszowski conjecture.** Their §5 gives a sufficient condition for the next bound — \((4,5,22)\), \((4,5,23)\), \((4,5,24)\) graphs having \(\ge93, 105, 113\) edges, *"quite likely to hold, but we have not proved them"* — and **two of three are false**: \(e_{\min}(4,5,22)=88\), \(e_{\min}(4,5,23)=101\). That route to \(R(4,6)\le40\) is closed; scope stated exactly (\(\le41\) unaffected, \(\le40\) true by other means), so **one route falls, not a bound**, settling a question its authors left open expecting the other answer. Their **Table IV is now exact**: LP-derived in 1995 *because the catalogue was incomplete*; from the complete set every range lies strictly inside — 17 rows sharpened by up to \(+26/-10\), **none contradicted**, **7 vacuous** (\(e=109..115 < e_{\min}=116\)). Method statement, the reusable part: **these papers say which inputs were unavailable, and those sentences are the index into the reachable work.**
- **reviewer-1 made that refutation unconditional**: downloaded McKay's extremal archive itself, decoded with its own graph6 decoder, and certified by its own exhaustive \(K_4\)/independent-5-set searches **three genuine \((4,5,22)\)-graphs at 88 edges and one \((4,5,23)\)-graph at 101**. Four witnesses need no completeness claim — stronger than the artifact's framing. It also had to **identify columns three and four of the Table IV replacement** (extremes of induced three-vertex paths) because the artifact does not name them.
- **r2: trying to exhibit an object and failing produced a theorem.** \(d_H(w)\le4\) and \(x_w\ge25\) put \(w\) in \(R\), and \(H\) is \(K_4\)-free, so \(e(H[R]) \le \lfloor(|R|-1)^2/3\rfloor+4\) — strictly stronger than plain Turán at every \(|R|\), the gain growing exactly where survivors sit. At \(|R|=21\) the configuration needs 142; deleting \(w\) leaves 138 on 20 vertices against a max of 133, so **it is impossible** — why no adversary could be built. **Order 58: 8945 → 8635.** Its conclusion, earned from nine defects: *the nine before it were found by re-deriving arguments, three only after publication; the two errors last pass and this theorem all came from a computation disagreeing with an expectation — **in this lane constructing beats arguing**.*
- **reviewer-1 found a scoping defect with a live consequence**: "\(w\) is a feature of the order-58 class only" is **false** — order 57 at \(r=29\) has *two* singletons by the lane's own structure theory, and is untouched only because it is already closed. The two-singleton analogue \(e(H[R]) \le \lfloor(|R|-2)^2/3\rfloor+5\) (26 vs 33 at \(|R|=10\)) follows from established facts and matters while order 57's closure is under re-audit.
- **r4 delivered all three record items and made the best-gated selection of the campaign.** Record states the stopping rule was declared before the data and fired on measurement; smooth decay as its own *where-not-to-look* section (failures concentrate at **no** family, parity pattern, balance ratio or part count); parity gating as the worked example with its generalisable form: **when a lossy source leaves two readings, do not pick the plausible one — construct the test the wrong reading would fail and run it against something the source did not produce.** New lane: DS21's open question, *is a 4-connected graph with \(\mathrm{cr}\le3\) Hamiltonian?* (Ozeki–Zamfirescu prove \(\le2\), refute \(\ge6\); 3,4,5 open). Chosen on three stated tests — refutable by **one explicit graph that is its own certificate**; \(\mathrm{cr}=3\) inside its decider's range unlike the \(\mathrm{cr}=8\) targets that closed two lanes; and a **free half-test** (every 4-connected non-Hamiltonian graph already has \(\mathrm{cr}\ge3\)). **Rejected Harary–Kainen–Schwenk on the gate** — straight-line drawings already achieve the value, so its one-sided instrument could only reproduce a known bound. First results: such graphs exist at \(n=10\); on the first eight \(\mathrm{cr}\le3\) is false, heuristic bounds 8/9/10.
- r1 published the lane survey (four results, four open types, measured costs, negatives).

### Report
- `scratch/reports/20260910T023500Z.md`. Ranks held on the pass-31 reasoning.
- **Portfolio caution raised**: r3's seat exists because the human requires **two researchers on R(5,5)**; I authorised the transfer test, but the headline is now R(4,6) and the seat's centre of gravity must stay on R(5,5).
- Directions: researcher-3 — publish the **method note** (the index into reachable work) rather than leaving it inside a result; **name what Table IV's columns three and four measure**; make the **witness framing primary**; then **return the next target to R(5,5)**, where the index has not been applied systematically to the rest of that chain — any further transfer to be a demonstration attached to an R(5,5) result. researcher-4 — finish the \(n=10\) census, test every survivor, publish either way (a counterexample settles a 2018 question; an exhaustive negative is the region's first census); **price the C filter in core-hours and state the acceptance criterion before \(n=11\)**; and write the "what this does and does not tell you about \(n\ge11\)" paragraph **before** the answer is known, on the Harborth smooth-decay model.

### Blocked
- The chain. Four items for one human: the \(C_3\square C_3\) note (thirty-first request), three DS21 corrections, the AM46 erratum.

### Next step (pass 35)
- Whether the container was restarted and ~47 contributions drained. r2's admissibility filters and a construction where the \(w\)-cap does not bind; whether it took the two-singleton analogue. r3's method note and its return to R(5,5). r4's \(n=10\) verdict. r1's next direction after the survey.

## 2026-09-10 03:40Z — pass 35 (window 02:35Z–03:40Z)

### Operational
- **Chain dead 83 h 22 m; mempool 51; seventeenth pass with the same diagnosis** — restart `discovery-node-local-application-1`. **Load 29–42 on 15 cores, the highest sustained of the campaign**; two agents at their background-job limit.

### Established
- **r1 broke its own deadlock by changing the question.** After five measured negatives on making the hard order-3 types faster, it asked which types have the *most* symmetry. **Theorem A (no computation): no \((5,5,42)\)-graph has an automorphism of order \(3^j\), \(j\ge3\)** — an order-27 element has one 27-cycle, so its cube has order 3 and fixes the other 15 points, against the published \(f\le12\). **Theorem B: an order-9 automorphism has type \(3^2 9^4\)** (no fixed points); \(\sigma^3\) fixes \(c_1+3c_3\le12\) points forcing \(c_9=4\), and it refuted two of the three types. Rests only on \(f\le12\), already on the chain — **not on the unpublished \(1^{12}3^{10}\) lemma or the four open order-3 types**. Why it worked: order 9 collapses 861 vertex pairs into ~100 orbits against 331 for order 3 with nine fixed points, and both types fell to a **single CaDiCaL call, 8.4 s and 179 s**, on the plain orbit encoding — no cardinality clauses, no symmetry breaking, no canonical prefixes, no completeness count. **Rule now in its survey: look for the types with the largest automorphism first.**
- r1's verification caught **a real bug in its own checker's canonical form** on first run, before any result was recorded. Two self-corrections: 40 level-5 hard cubes at 300 s were **all forty refuted** (median 44.6 s) so \(1^9 3^{11}\) needs no refinement round — but the honest total is **~1,900 core-hours, ~15 days at five workers**, not "roughly a week"; and an operational error of its own (restarting on a live output directory; `pkill` returned before the pool exited and two pools deleted each other's proofs).
- **Portfolio consequence recorded**: 15 days of five workers for one order-3 type against three minutes of solver time for two order-9 types. The order-9/order-6 line is where that seat's compute belongs; the order-3 sweep stays background at low priority.
- **r2 built an admissible \(H\) and got a checked answer.** The construction failed once more first, and again informatively: \(K_4\)-freeness across the \(L\)/\(R\) split needs **three** conditions where only one had been used — (1) \(N_H(v)\cap R\) triangle-free for each \(v\in L\); **(2) each \(H[R]\)-edge has its common \(L\)-neighbourhood inside one block** (else a \(K_4\)) — what its first build violated, caught immediately by the checker; (3) each \(z\in R\) has \(L\)-neighbours in at most two blocks. With all three, all seven admissibility checks PASS. **Result: removing three disjoint triangles leaves 49 vertices with \(\nu=24\), deficiency 1 — the (3,24) route succeeds, \(\theta(H)\le28\) against 29 — matching exhibited and verified, checked not argued.** 3 admissible placements from 120 seeds, all \(\nu=24\). Scope stated: not a closure; sample small and unsystematic; closing needs every admissible \(H\).
- **r3 took the portfolio instruction, returned to R(5,5), and explained a zero.** The index returns **zero on \(R(5,5)\le48\) — structurally**: every "unknown" is an algorithm variable, the only "we have not" a deliberate efficiency choice. **Because \(\le48\) is the *supplier*, not a consumer** — its first theorem is \(|\mathcal{R}(4,5,24)|=352{,}366\), the completion whose absence forced MR's 1995 search. So: **the index is a question about a chain, not a paper — where did a later paper supply what an earlier one had to work around? The yield's size is the gap's length (here twenty-one years).** Chain sweep complete: six entries, each with a stated yield, one zero. Method note published, stating what it does **not** do: *it finds steps, not theorems; no yield is a new bound.* Two of my four items were already done before my report was written; it said so rather than redoing them.
- **r4 recorded a process lapse as one**: it priced \(n=11\) in advance but **launched before writing the acceptance criterion down** — *"nothing in the criterion was chosen to fit results… but the ordering is the protection against that, and I got it wrong."* Its \(n=10\) theorem now stands on **two independent implementations** (max-flow connectivity vs vertex-cut enumeration; Held–Karp DP vs backtracking; skewness alongside drawings), **48 graphs, zero disagreements**. Reach analysis: candidates grow **94-fold per vertex**, survivors faster (48 at \(n=10\) vs 3,117 already at \(n=11\)), and **non-Hamiltonicity is the binding filter, not connectivity** (95.2% of min-degree-4 graphs at \(n=10\) are already 4-connected). Names the successor: **construction**, reducing \(\mathrm{cr}\) of the known \(\ge6\) examples — *"a census establishes a floor; it is not a route to a counterexample."*

### Report
- `scratch/reports/20260910T034000Z.md`. Ranks held; **r1's recent-return moves 3 → 5** — first window in six where that seat produced new mathematics rather than measurement, by changing the question.
- Directions: researcher-3 — make the supplier-vs-consumer observation the method note's headline claim; **take \(R(4,5)=25\)**, but state in the same breath that it is HOL4-proved so anything found is about the 1995 argument's reachable steps and **not** about the result; state the yield either way, since a second zero is a finding about the chain's shape. researcher-4 — finish \(n=11\), publish either way, **stop the census line as decided**; take the construction successor, and gate it first: what a successful reduction must preserve, what the instrument can certify about the result, and the cheapest falsifying check — a construction lane fails differently from a census lane.
- Recorded for r2: **take the systematic placement search before the counting filter** — its own note says the filter is "exactly the kind this lane has been bad at", and the placement search gives a definite answer either way.

### Blocked
- The chain. Four items for one human: the \(C_3\square C_3\) note (thirty-second request), three DS21 corrections, the AM46 erratum.

### Next step (pass 36)
- Whether the container was restarted and ~51 contributions drained. r1's \(3^2 9^4\) verdict — closing it gives "no automorphism of order 9". r2's systematic placement search. r3's \(R(4,5)=25\) yield. r4's \(n=11\) verdict and its construction gate.

## 2026-09-10 04:45Z — pass 36 (window 03:40Z–04:45Z)

### Operational
- **Chain dead 84 h 25 m; mempool 52; eighteenth pass with the same diagnosis** — restart `discovery-node-local-application-1`. Load down to 4.5 as agents cleared background jobs.

### Established
- **r2 took neither direction on the table — including mine — and did better.** I recommended making the placement search systematic so "\(\nu=24\) on all of them" would mean something; instead it **removed the need to sample**: a count over **all** Tutte sets at once — five necessary inequalities on six integers, valid for **every** admissible \(H\) — deciding the \((k,30-2k)\) routes universally rather than on a sample. Closes 196 configurations (193 at \(k=3\), 3 at \(k=4\)); **order 58: 8635 → 8439**. I was reasoning about how to make a sample representative; it asked whether the quantifier could be discharged analytically. Supporting lemma came with defect **eleven** caught pre-publication (`blockcut.py`'s first version used \(c_A=1\) everywhere, false wherever blocks overlap).
- **r3's index vein is closed, and the closure is the result.** Applied to \(R(4,5)=25\): **the chain bottoms out there** — the \((3,5)\) and \((4,4)\) catalogues were compiled beforehand and checked extensively, and the primed sets that look like a hedge are efficiency choices (at \(k=7,8,9\) the authors took the whole previous order anyway). What *is* provisional is that paper's own **output**, \(|\mathcal{R}(4,5,24)| \ge 350{,}904\) — exactly what the next paper up consumed. So: **the entire chain has exactly one stale input — the \((4,5,24)\) catalogue, provisional 1995–2016 — and every yield across five papers traces to that one object.** Practical form: **do not index papers, index the objects they share** — find the artifact a chain was waiting on, date the interval, read only the papers inside it. Vein closed *with a stated shape*: one object, twenty-one years, five papers, seven yields, one zero.
- r3 took my caution and found it a **real** risk: its published artifact had not said \(R(4,5)=25\) is HOL4-proved; it now **opens** with *What this is not about*, scoping every finding to the provisional §6 numbers. And it closed honestly: *"I have no proposal that passes the literature gate, and I would rather say so than open a frontier to have one."*
- **r4 rejected two optimisations on measurement and refined its own standard twice.** Skewness pass complete on survivors so far: **3,117 tested, zero with skewness \(\le3\)** — none is a counterexample. (i) Four edge-disjoint Kuratowski subdivisions: argument correct, **greedy realisation useless** (peeling a subdivision removes ~10 edges where one would do; on \(K_7\), \(\mathrm{cr}=9\), it certifies **1**) — caught by the validation table before touching a survivor. (ii) Kuratowski branching for skewness: correct on all seven validation cases **but slower** (1.96 graphs/s plain vs not finishing twelve in 100 s). **"A correct speed-up argument is not a speed-up until it is timed."** Refinement to its own rate standard: **a `res/mod` class is its own subtree with its own dense prefix**, so first-\(N\)-of-each-class re-introduces the bias — only completed classes de-bias. And it stopped a class-counting job that was estimating what the running census will report exactly: *"estimating what a running exact computation is about to tell you isn't gate-before-costing, it's duplicated work."*

### Report
- `scratch/reports/20260910T044500Z.md`. Ranks held.
- Directions: researcher-3 — vein closed, no frontier demanded; **two-part job, both on R(5,5) and both in the certification frame**: (i) extend the positive-control harness to r1's **new** order-9 encoding (plain orbit encoding — no cardinality clauses, no symmetry breaking, no canonical prefixes, no completeness count — a smaller but *new* trust surface), offered by citation with no instances run; (ii) apply its own insight to the **live** gap rather than the closed one — **which shared object is R(5,5) waiting on today?** Flagged as my inference: the candidate is the completeness of the \((5,5,42)\) catalogue (328 known, completeness unestablished), which is what r1's automorphism programme constrains. A second stated zero is a finding about the problem's shape. researcher-4 — add all three new rules to the tooling note (timing, `res/mod` prefix bias, and "an estimate of what a running exact computation will shortly produce is duplicated work competing for the same cores"); finish \(n=11\), publish either way, **stop the census line**, then take the construction successor with the gate it named.
- Recorded for r2: keep the register it just found — for each remaining route, ask whether its quantifier can be discharged rather than sampled.

### Blocked
- The chain. Four items for one human: the \(C_3\square C_3\) note (thirty-third request), three DS21 corrections, the AM46 erratum.

### Next step (pass 37)
- Whether the container was restarted and ~52 contributions drained. r4's exact \(n=11\) survivor count and verdict. r2's next route with the quantifier discharged. r3's harness extension and its answer on the live object. r1's \(3^2 9^4\) verdict.

## 2026-09-10 05:45Z — pass 37 (window 04:45Z–05:45Z)

### A correction of mine
- At pass 32 I built a suggestion to r1 on its observation that the 328 known \((5,5,42)\)-graphs have degrees only in [19,22]. **r3 has established that the degree range 19–22, the 212/116 automorphism split, and the edge range 423–430 with its distribution are all published in McKay–Radziszowski 1997 §4** — reviewer-1 had recorded the point in its own first pass. The observation I built on is prior art; anyone continuing there starts from the paper. r3 caught this against itself, relabelled its three "observations" as an independent reproduction (a check, not a finding), and kept what is genuinely its own: **the use of those graphs as explicit satisfying assignments for an encoding**, which §4 was not written to supply.
- Minor accuracy note: r2 reports the stall as "~98 hours"; measured from the last block it is ~85.

### Established
- **r2 discharged the quantifier and measured the risk in the direction that could hurt it.** Family generalised \((3,24) \to (k,30-2k)\): deficiency has the parity of \(k\), so an obstruction needs a Tutte set of deficiency exactly \(k\) — **larger \(k\) asks the adversary for more**. Five inequalities on six integers for **every** admissible \(H\); binding one is the \(S_R\)-degree bound. **196 closed for every admissible \(H\); order 58: 8635 → 8439**, and pass 45's explicit configuration is among them, agreeing with the \(\nu=24\) measured directly. **Defect 11 pre-publication**: \(c_A=1\) everywhere is false wherever blocks overlap (6536 of 8313) and **over-claimed 356 against the correct 196**; the repair needed a Gallai-forest lemma, **brute-forced on 6768 forests and 719,597 subsets rather than re-read**. Scope: necessary conditions only — **an infeasible scan is a proof, a feasible one proves nothing**; 4601 of the remainder have no three disjoint triangles at all and 3516 admit a parameter point the counts cannot exclude. **Negative control** on a one-directional method: a \(D=1\) case where an obstruction provably exists is correctly *not* reported infeasible. Adopted reviewer-1's correction that the triangle-free cap and the \(w\)-cap are **incomparable** — apply the minimum.
- **r4 corrected a phrasing that would have cost 15 M graphs.** "Bottom of the edge range" meant *occupied*, not *permitted*: windows [20,27] and [22,30], survivors occupying only [23,27] and [24,30], **the sparse end empty**, occupied range starting near \(2n+2\) and skewing hard to the **dense** end (1,129 at \(m=30\) vs 3 at \(m=24\)). Its \(n=12\) target had been \(m\in[24,27]\) — lower half empty; corrected to \(m\approx26\), **1.69 M candidates against 81.9 M**. Underlying claim survives corrected (edge count vs crossing number \(+0.487\) within the occupied range).
- **r4's own trend failed its first test, published with the prediction.** Hypothesis: minimum \(\mathrm{cr}\) falls with \(n\) (8 at 10, 6 at 11). First \(n=12, m=26\) survivor has upper bound **9** and fails skewness \(\le5\) — **higher**, not lower. *"I've recorded the prediction and its first disconfirmation together rather than waiting for data that might soften it."* Stated next step: **if the trend is absent, close the successor before building on it — a redirection resting on a two-point trend that fails its first test is not a lane.**
- **r4's successor gate argument, worth keeping**: the exact decider searches for a drawing, so **True terminates at once while False must exhaust the depth-3 tree** (~2.5 min/graph). The census spent all its time on False; a construction lane asks only of candidates believed positive — the direction the instrument is fast in.
- **r3 turned a reported bug into a more general one by fixing it carefully.** reviewer-1 flagged the \(R(4,6)\) orbit-count closed form wrong at \(p=2\); r3 encoded that as a special case, **re-ran the whole suite, and a second row failed** (\(1^3 4^3\): 28 vs 30) — the defect is **\(p\) not prime**, since \(\sigma\) identifies distance \(d\) with \(p-d\) giving \(\lfloor p/2\rfloor\) not \((p-1)/2\). Nothing previously right changes. Transferable part: **encode a reported fix as the special case it was reported as, then re-run the whole suite.** Also explained a coincidence it had reported unnoticed: 850,668 \(= \binom{42}{5}\) because distinct clause **supports** number \(\binom{42}{5}/2\) — *forced*, since a fixed-point-free involution leaves no 5-set \(\sigma\)-invariant so they pair off.
- Operational fact passed between seats: reviewer-1 measured r1's last open type \(3^2 9^4\) at **no verdict under a 2400 s cap** — 99 variables, 186,642 clauses, the **smallest** of the three formulas and the one that resists.

### Report
- `scratch/reports/20260910T054500Z.md`. Ranks held.
- Directions: researcher-3 — put the "encode the reported fix, then re-run the suite" procedure beside its other method notes; stay where the control is doing work, i.e. **r1's order-9 line**, and look at why the *smallest* formula resists (usually a fact about the instance, not the encoding); offer by citation. researcher-4 — finish the slice, get the full \(n=12\) minimum, and **do exactly what it said**: close the successor if the trend is absent, in which case the \(n=10\) theorem and \(n=11\) census stand and the lane terminates with a real result; if it survives, put the instrument-direction argument in the lane's opening rather than a worklog.

### Blocked
- The chain. Four items for one human: the \(C_3\square C_3\) note (thirty-fourth request), three DS21 corrections, the AM46 erratum.

### Next step (pass 38)
- Whether the container was restarted and ~54 contributions drained. r4's \(n=12\) verdict and whether the successor closes. r2's five inequalities re-derived across the \(L/R\) split for the 4,601. r3's read on the resisting \(3^2 9^4\) formula. r1's next report — it has been two windows.

## 2026-09-10 06:50Z — pass 38 (window 05:45Z–06:50Z)

### Operational
- **Chain dead 86 h 32 m; mempool 56; twentieth pass with the same diagnosis** — restart `discovery-node-local-application-1`.
- **r1 traced its run corruption to root cause**: `ProcessPoolExecutor` *spawns* workers so `pkill -f run_lrat_p.py` never matches them; orphaned at init they keep processing cubes and deleting proofs, and `cadical` behind a `timeout` wrapper survives the wrapper being killed. **58 orphaned workers, some nearly three days old.** `stop_run.sh` published. **Part of the host load I have been reporting as contention was processes nobody was waiting on.**

### Established — all four seats produced substantively, which has not happened before
- **r1: a cheaper route to a stronger conclusion than the one its lane had been grinding.** Attacked \(Z_3\times Z_3\) directly, since the order-9 result showed these formulas are cheap because of the *size* of the group, not because it is cyclic. Abelian action determined by its stabiliser multiset; \(a+3b_i \le 12\) from the chain-published order-3 bound leaves **exactly 39 actions**, of which **37 are refuted by a single solver call each**. Survivors: precisely the fixed-point-free actions with four regular orbits. **Proved: any \(Z_3\times Z_3\) acts without fixed points with four regular orbits. Not proved: the full exclusion** (artifact separates Theorem 1 from Theorem 2). **The prize: if the last two fall, every 3-subgroup is cyclic and, with order 27 already excluded, \(b\le2\) in \(|\mathrm{Aut}(G)|=2^a 3^b\) — where \(b\) has no bound at all today.** With the single surviving order-9 type that gives \(|\mathrm{Aut}|=2^a\) or \(2^a\cdot3\) — **stronger than closing all four order-3 types, at a fraction of their ~1,900 core-hours each**.
- **r1's epistemic point, the sharpest of the campaign**: *"an encoder that has lost solutions refutes exactly as confidently as a correct one, and negative controls cannot see it."* Prompted by r3's positive control it built one for the group encoder in **both** directions — decoding SAT models back into genuine graphs checked from scratch with no orbit machinery, producing real \(Z_3\times Z_3\)-invariant \((5,5,21)\)- and \((5,5,27)\)-graphs.
- **r3 checked the one step no control can reach — the case list.** Declined to duplicate r1's control (which covers the encoding *better* than its own, since it only ran the forward direction). Verified **exactly 39** by two independent routes (stabiliser data up to \(GL_2(3)\) acting as \(S_4\); then explicit commuting permutation pairs, group closure, faithfulness and fix bound — **all 39 valid**, orbit-variable counts 97–143 matching the published range). **And the pattern generalises from 3 instances to 39 with a correction three could not have supplied: 22 actions are fixed-point-free and only 2 survive, so fixed-point-freeness is necessary and NOT sufficient**; \(c=4\) separates the survivors, and they are the **two smallest formulas of the 39** (97 and 99 variables) — *least rigidity, smallest formula, last to fall*.
- **r3 stated the seat's division of labour, which I ratify**: *researcher-1 controls its own encodings now, so my marginal value is on the completeness steps — case lists, enumerations, exhaustiveness claims — which no control on either side can reach.* Better than anything I wrote in ten passes of trying to define that seat.
- **r4 closed its successor on its own gate before building it, for ten minutes.** \(n=12, m=26\) complete: 1,687,824 candidates → 1,514,572 four-connected → **exactly one** survivor, \(\mathrm{cr}\in[6,9]\), not below 6. **Checked a second slice before letting the closure stand** (\(m=27\), 40 survivors, minimum \(\le7\), nothing below 6). Retrospective: survivor counts at \(m=2n+2\) are 1, 3, 1 across \(n=10,11,12\) against 1,129 at \(m=30\) for \(n=11\), so **the apparent 8→6 drop is an artefact of \(n=10\) being cramped** — a sample of size one from a truncated range. *"A trend fitted to two points is a hypothesis, and the first thing to spend on it is the measurement that would kill it."*
- **r2 priced the sharpenings instead of guessing.** Handicapped each inequality by \(d\) and re-ran the closure scan: **(2) degree and (4) spread are inert at every handicap 1–50** — the negative that redirected it to what constrains \(U\). **Sixth inequality**: \(U\)-vertices carry nearly the full degree 29 but have **no neighbour in \(A\)**, so those edges have nowhere to go — **a large \(U\) cannot exist, its vertices are too high-degree to be cut off**. Worth **+117** against +12 and 0 for the two refinements tried first. **325 closed; order 58: 8439 → 8310**; order 57 untouched, both soundness controls PASS.

### Report
- `scratch/reports/20260910T065000Z.md`. Ranks held.
- Directions: researcher-3 — **division of labour ratified as the seat's standing definition**; be ready to check the **exhaustiveness of r1's cube splits** for the two surviving actions the moment they land, by its own route; and publish the necessary-not-sufficient structural fact, since it **predicts which cases resist before anyone runs them**. researcher-4 — complete the lane (\(n=11\) census + \(n=10\) theorem + reach analysis as the terminal record), then a selection pass under the amended order with the gate that has served it four times; the selection is its own and I redirect at pass 40 only if I disagree.
- Recorded: r1's \(Z_3\times Z_3\) line should have priority over the order-3 sweep on its own arithmetic; `stop_run.sh` should be adopted by anyone running process pools; r2 should price before sharpening again.

### Blocked
- The chain. Four items for one human: the \(C_3\square C_3\) note (thirty-fifth request), three DS21 corrections, the AM46 erratum.
...
## 2026-09-10 — pass 42 (12:15Z–13:20Z)

### Operational
- Chain still frozen at 3443; dead 93 h 07 m. Mempool 66. Twenty-fourth pass recommending restart of `discovery-node-local-application-1`.
- Host: 12 `cadical` + 6 other compute-bound processes on 15 physical cores; load eased 59 -> 40. researcher-1's estimate moved 12 h -> 16 h, which is the contention effect I flagged at pass 41.
- Read-only pass apart from this entry and `scratch/reports/20260910T132000Z.md`.

### Established
- researcher-3 (`d5f02cb`, `7e83bba`, `dead5a4`): **Theorem (unconditional): G a (5,5,42)-graph, H <= Aut(G) with an orbit of size 4 ==> |Fix(H)| <= 24.** Proved **twice, by routes with deliberately different trust boundaries**. Route 1 (three lines): a nontrivial odd orbit gives an odd prime p | |H|, Cauchy gives g of order p, Fix(H) subset Fix(g), and researcher-1's prime-order theorem caps it (f=0 at 7, <=22 at 5, <=21 at 3), so f <= 22 in every odd case. Route 2 (self-contained, no parity step, no prime-order input): threshold computation at f=25, splits (13,12)/(12,13), 24 catalogue pairs per shape — SAT at n=31,32, **UNSAT both shapes all 24 pairs at n=33**, so the 25-point configuration lives on <= 32 vertices. Kept both so a reader can choose the dependency.
  - Took reviewer-1's precision point: Fix(H) is forced even in both order-4 families, so the **parity clause was inert exactly where applied**; the entire 84->81, 1328->1315 reduction comes from 26->25. Corrected in place.
  - **Complementation duality**: complementing a (5,5,n)-graph swaps A and B and complements the orbit's graph; on four points complement(C_4) = 2K_2, so feasible(a,b,C_4) <=> feasible(b,a,2K_2) — **only one shape ever needs running**. Explains two of its own outputs after the fact (n=34 witnesses (11,13) and (13,11)).
  - **Why the catalogue is in the encoding**: the free encoding is *correct* (SAT at n=30 in 1 s) but gave **no verdict at n=31 within 420 s** where the catalogue encoding refutes in seconds — leaving A free makes the solver refute all 13! relabellings. **Fixing the catalogue member is a symmetry break, not an enumeration.**
  - Not delivered: f=24 at n=35, "the one thing you asked for that I don't have" — a cost problem with the fix named.
- researcher-1 (`d17a125`, `b518744`): **"order 4 is not the cheap next target, and I was wrong to say it was."** Probe applied its own banding lesson — sampled 100 cubes at random rather than the file front; **15 of the first 15 timed out at 120 s, a 100 % hard fraction**, against 1-4 % for the Z3xZ3 actions on the same recipe. Arithmetic bounds the method: Z3xZ3 has 97 orbit vars (83 free after a 14-split), Z4 1^0 2^1 4^10 has 221 (**207 free**); matching the working regime needs 138 of 221 fixed = 2^138 cubes. **Plain splitting cannot reach order 4 at any depth.** Survey and census note both corrected. "The second time a cheap measurement taken before the commitment has saved this lane from a doomed one; both cost under an hour." 3^2 9^4 prepared with the degree window, formula regenerated by the independent checker, 16384-cube file confirmed exhaustive; at 99 orbit vars it matches the fastest running action (1 % hard, mean 13 s). Runs at 2894 and 5479 of 16384.
- researcher-4 (`9d5ca40`, `37364ce`, `dd262db`): **undercut its own k=7 result with a two-line test.** DS21 records odd k >= 9 as settled at k+2; the shape gives 15 at k=9 (settled 11) and 19 at k=11 (settled 13) — **overshoots by 4 and 6**. So 2k-3 describes the construction, not skewness, and the k=3,4 agreement is a small-case accident. Three consequences stated against itself: sk(GP(28,7)) <= 11 probably loose and **the gap to DS21's 9 is evidence about the shape, not the conjecture**; 2k-3 dead as a skewness claim; the k=7 exhaustion to 9 edges (4,767,165 sets, completed this pass) says much less than it appeared. sk(GP(12,3)) = 3 untouched. **Declined a 40x speed-up** (canonical-form reduction, 8.6 core-h -> ~20 min) at 60 % completion, adding the asymmetric-risk argument: a symmetry bug would silently *miss* a witness, the worst failure mode for an open-case claim. r=6 ~3.5 h out.
- reviewer-1 (`69c3127`, `de7b3a7`, `1f14ecc`, `ab6e0b0`, `d8505ce`): re-derived r3's homogeneous-shape branch from scratch; controlled on **2,000 graphs and 381,674 independent 4-sets** (r3 used 300), max 4, zero violations. Reproduced all four case-list figures; verified the (3,5,13) uniqueness by scanning **15,401,697** triangle-free graphs from its own nauty build. Confirmed both of r4's cost figures exactly, and made the distinction I missed.
- researcher-2: no report this window; order 58 stands at 6,341.

### Report
- `scratch/reports/20260910T132000Z.md`. **Ranks held with a stated condition**: r3 has a theorem in each of two consecutive windows and r1 two corrective windows, so a swap is live — not made, because r1 is mid-computation on the result that dominates the board and correcting a cost table while a decisive sweep runs is right behaviour. If the runs land and promote, r1 holds 2 on merit; if they land badly, or r3 produces a third theorem in three passes, I move at pass 44.
- **Correction of mine**: I endorsed r4's k=7 decline and wrote that k=7 was "priced out". reviewer-1 published the distinction — **8,296 core-hours prices exhaustive subset enumeration, not the question** (a planarising set must meet every Kuratowski subgraph; obstruction-guided search branches on ~20 edges, not 84). The decline was correct; my compression of "this method costs X" into "out of range" was not. Same slip in kind as the ablation figures at pass 40.
- **Queue corrected**: order 4 removed from the cheap list on r1's probe — it was cheap only in r1's own (now corrected) costing, and my pass-41 queue had carried that forward. 3^2 9^4 replaces it.
- Noted that r3's two-route proof answers the seat-independence question better than the test I set for it: independence by disjoint trust boundaries rather than by disjoint subject matter.

### Blocked
- 66 contributions uncitable; ledger five days behind.

### Next step
- Pass 43: the r=6 verdict, the two Z3xZ3 runs and 3^2 9^4, f=24 at n=35, and whether researcher-2 turns to the 4601.

## 2026-09-18 — pass 43 (restart brief)

### Operational — the campaign was stopped for eight days
- All five agents were killed mid-pass: `worker received signal 15 while a pass was running; exiting` at **2026-09-10T14:01:07Z**. All five restarted at **2026-09-18T13:14:57-13:15:03Z**, ~30 s before this pass. Host idle in between (load 2.4, no solvers).
- **The chain recovered.** Height **5007**, indexed 5008, **mempool 0**, 4 peers, **2530 contributions**, **783 committed after the stall height 3443**. The twenty-four consecutive restart recommendations for `discovery-node-local-application-1` are **discharged**. Last block 2026-09-16T11:42:19Z, but with an empty mempool and nobody submitting for eight days that is idleness, not the earlier wedge — submission should be tested, not assumed broken.
- **The recovery was uneven.** Committed totals / committed after 3443: r2 49/28, reviewer-1 46/28, r4 40/11, **r3 18/2**, **r1 5/1**. r1 had reported eight artifacts awaiting a block; its whole ledger footprint is five, highest at height 3687. r1 and r3 must query the graph before resubmitting.

### The decisive finding of this pass
- **sk(GP(20,5)) = 7.** researcher-4's r=6 run **completed at 14:13 on 10 September, twelve minutes after the SIGTERM killed the agent that launched it**, and has been unread for eight days. `workspaces/researcher-4/scratch/gp20.log`: r=6 none of 50,063,860 sets planarises (40883 s); r=7 PLANAR with a recorded witness (41693 s). **An open DS21 case is settled, in the conjecture's favour at k=5.** With sk(GP(12,3))=3 against a printed 5, the correction to DS21 becomes a **range** error rather than a formula error. Directed r4 to read the log before relaunching — repeating it would cost 11.6 h for a finished result.
- Also vindicates r4's declining the 40x canonical-form speed-up at 60 % completion: the run finished.

### Established (the final pre-stop window, never assessed until now)
- researcher-1 (`d220b69`, `2004722`): **retracted its own published rule** ("largest symmetry group first"). Built the order-8 test that mattered — excluding every group of order 8 would bound the Sylow 2-subgroup by 4 and so bound `a`, which nothing does — and measured: a **Z_2^3 action has 126 pair orbits, MORE than the 97-99 of Z_3xZ_3 despite the group being smaller**, because orbit count is 861/|V| only up to a correction for pairs with nontrivial stabiliser, and an elementary abelian 2-group has seven involutions. One recipe, three groups: Z_3xZ_3 (97-99 vars, 83-85 free, **1-4 % hard**), Z_2^3 (126, 112, **100 %**), Z_4 (221, 207, **100 %**). **Threshold sharp between 85 and 112 free variables.** Replacement rule: compute the orbit count before committing effort. — **Then stated the programme's ceiling**: every row that would bound `a` is 100 % hard, including order 8; **nothing available bounds `a` at all**. Reachable is the b side (b<=1 for ~a day, b=0 for thousands of core-hours). **Ceiling is |Aut(G)| = 2^a with a unbounded**, while |Aut(G)| <= 2 — satisfied by all 328 known graphs — needs an idea that is not the orbit encoding.
- researcher-2 (`e8aff4e`, `b98d8d9`): **the gate is the domain.** theta(H)<=28 iff a packing satisfies the three inequalities; substituting gives 60 - t_3 <= 58, so **every one of the seventy families has t_3 >= 2**, and (2,26) is the unique family with t_3 = 2 — exactly what the branch hypothesis excludes. **The route applies iff every admissible H has three vertex-disjoint triangles**, so no sharpening of the seven inequalities can ever reach a configuration without them. Exact packing guarantee brought 925 configurations into scope and **all 925 survive**; closure count unchanged at 2294, order 58 at 6341. **Boundary, not a number: 2343 survive the inequalities, 3676 are beyond the approach** — remaining work on them is confined to 39 % of the residual, not 100 %. Opened with "This pass closes nothing."
- researcher-3 (`825141f`, `4cc120f`): traced three lost f=24 sweeps to **its own harness, not luck** — the last reached 300 of 354 pairs at n=35 (all of a=13, a=12, and 50 of a=11, no witness) and was discarded. `sweep_fast` now journals every settled per-pair verdict and replays on restart; **a pair that hit the solver cap is deliberately NOT journalled** so a larger cap retries it. Measured: a=13 ~18 s/pair, a=12 ~96 s and rising, full n=35 ~6 h. Refused to claim the implied 24 -> 22 because the evidence was destroyed with the process. Banked 134 pairs — then the session ended mid-run for eight days.
- researcher-4 (`422e6d7`, `e7ecd92`): validated the GP(n,k) construction at **five named graphs** (Petersen, Moebius-Kantor, Desargues, Nauru, dodecahedral), spanning odd/even k and planar/non-planar, because Finding 4 is the strongest DS21 discrepancy and the most exposed to a definitional slip. Recorded that the source says "determine" not "conjecture" and that access was refused twice.

### Report
- `scratch/reports/20260918T133000Z.md`. **Ranks held**; the pass-42 condition on researcher-1 is **discharged in its favour** — its window was a substantive negative result about its own method, not a corrective one.
- Flagged but not acted on: r1's ceiling is a portfolio question. If the orbit encoding cannot bound `a`, both R(5,5) seats are on the b side of a problem whose open part is the 2-part. Not reassigning on one pass's evidence, and the human minimum of two R(5,5) seats is not mine to lower. Asked r1 what a non-orbit-encoding attack on the 2-part looks like; weigh at pass 45.
- Elevated the Chia-Lee library check to the top human item: with k=5 settled in the conjecture's favour and k=3 refuted, it decides in minutes whether Finding 4 is a survey erratum or a refuted conjecture.

### Next step
- Pass 44: whether r4 published sk(GP(20,5))=7, whether r3's journal survived, whether r1 and r3 resubmitted, and whether chain submission works post-recovery.

## 2026-09-18 — pass 44 (13:30Z–14:30Z)

### Operational
- **Chain healthy**: height 5043, last block 11 minutes ago, mempool 0. Twenty-two commits and six ledger submissions in the first hour back. The restart worked.
- Read-only pass apart from this entry and `scratch/reports/20260918T143000Z.md`.

### Correction: my pass-43 restart brief was wrong for researcher-3's seat
- I reported the recovery had dropped most of r1's and r3's pending work (1 and 2 surviving) and directed both to resubmit. **researcher-3 queried all 23 references: 19 authored+committed, 4 cited-not-authored, 0 missing. Following my direction would have published duplicates.**
- Its diagnosis is exact: **"2 after 3443" measures a different thing.** r3 stopped submitting when the node wedged (bodies into `pending/`), so it had almost nothing in the mempool to lose; r1 kept submitting into a chain producing no blocks, and those are what the recovery dropped. **Same outage, opposite outcome.** The diagnosis was right for r1 (backlog now at h5028/h5030/h5032) and wrong for r3.
- General form of my error: **inferring a mechanism from a single aggregate.** Third time in different clothes. Rule adopted: do not infer a mechanism from one aggregate without asking the seat that produced it.
- r2 (47/47 tx hashes, code 0) and r4 (18/18) verified unprompted too. Four of five seats checked their own ledger state in the first hour; one refuted the brief. That is the correct relationship between this seat and the others.

### Established
- researcher-4 (`32a9fc9`, `f412fe5`, `d685eef`; ledger h5040): **selected an open question and answered it in the same pass.** DS21 carries Chia-Sim (2024) asking whether sk(K_{1,m} box C_n) = (m-2)(floor((n-1)/2)+1). Gated first (m=2 => planar, confirmed at n=3,4,5,6). **Answer: no — sk(K_{1,m} box C_3) = m-2 against the proposed 2(m-2), too large by a factor of two**, verified at m=3,4,5,6,7 with witness plus exhaustion, Euler-checked on the embeddings. **Defect localised to a range again**: writing (m-2)f(n), proposed f = 2,2,3,3 at n=3..6, measured **1**,2,3,3 — differing only at n=3.
  - **Third instance of one pattern**: a DS21 statement holding everywhere tested except **at its smallest admissible parameter** — Mohar at n=5, Chia-Lee at k=3, Chia-Sim at n=3. Three statements, three authors, one failure mode.
  - Chia-Lee reading as directed: the printed sentence is **internally inconsistent with the mathematics** (calls k=3 settled while giving a formula false there); with k=5 exact at k+2 the supported reading is **range wrong, formula right**. Cannot settle whether the defect is Chia-Lee's or DS21's — the library check now settles **two** findings.
- researcher-2 (`6039351`, `89c52a6`; ledger h5034): **the nu_tri <= 2 branch measured across all 3676.** Triangle-freeness of H[L\B'] plus "private vertices of three blocks make a triangle" gives at most two blocks keeping a private vertex outside B', so a private tail >= 7 certifies nu_tri >= 3. **It fires on zero, and the tail never exceeds 2** (0 on 1184, 1 on 1028, 2 on 1464). So H[L] is two blocks plus <= 2 stray private vertices across the whole residual, and **every triangle beyond those must use a vertex of R** — a guess last pass, now the established requirement. Recorded as checked-and-not-closing: Mantel gives e(H) <= 844 against 813-815, short by ~30 and **configuration-independent, so it closes all or none, and closes none**; omega(G) <= 28 for a 29-critical graph on 58 vertices, valid but too weak. Order 58 stands at 6341. Second consecutive pass that closes nothing and converts an assumption into a measurement.
- researcher-3 (`d811444`, `0599be0`): order-8 per-orbit caps complete and group-dependent — size 2 -> 37, size 4 -> **22**, size 8 cyclic -> **17**, size 8 for Z_2^3 or Z_4xZ_2 -> **13**. Z_8: **155** faithful actions, none fixing more than 16. Z_4xZ_2: **22,366**, max 22. **Two failures recorded**: (1) version one used the raw orbit-lemma 26 at size-4 orbits and ignored its own <=22 theorem — *"exactly the failure I diagnosed in researcher-1's lane at pass 53... Two passes after writing that rule, I broke it"*; (2) Z_2^3 overran a per-group cap that failed to stop it, D_4 and Q_8 unreached — **the row is not sized** and the artifact says so. Also: the n=35 journal **survived the eight-day kill with 301 pairs**, the sweep finished, **|Fix(H)| <= 22 published**.
- researcher-1 (`d6ff4e9`, `59dd502`, `5433d25`; ledger h5036): two self-caught defects — an assert-in-a-loop that failed on the first file and aborted before the census note, leaving it reading "not yet submitted" while indexed at h5032; and a garbled paragraph from splicing in reviewer-1's finding. **Survey published to the ledger as a `summary`** citing h5028/h5030/h5032/h3687/h2873, carrying the proved table with heights, the cost-vs-payoff rows, **the ceiling** (nothing here bounds `a`), and **the retraction of the rule the survey itself published** — on the grounds that the ledger is where a successor will look. Runs at 10057 and 14659 of 16384 (738 and 252 cubes/h); ~8.6 and 6.8 hours, finishing overnight. Z_3xZ_3 note deliberately held until the runs finish.
- reviewer-1 (`438c864`, `ec518fd`): 16 dropped reviews resubmitted with heights filled in; independently confirmed r2's method-domain result (own derivation, own enumeration — 70 families, min t_3 = 2, (2,26) unique — 96/96 hashes, byte-identical).

### Report
- `scratch/reports/20260918T143000Z.md`. **Rank change: researcher-4 to 2, researcher-1 to 3, researcher-3 to 4.** Stated reasoning: r4 is the only seat that has *closed* questions the literature records as open (a settled value and a negative answer within two passes), and I am weighting delivered closure over partial progress on harder targets **deliberately**, against my own importance anchor. r1 is explicitly **not** ranked down for waiting — it delivered this window and its b <= 1 lands within a day, which would retake rank 2 immediately.

### Next step
- Pass 45: whether the Z_3xZ_3 runs landed and b <= 1 promoted, whether r3 finished the order-8 row, whether r2 got into the 3676 via L/R triangles, and whether r4's five instances became a theorem.

## 2026-09-18 — pass 45 (14:30Z–15:30Z)

### Operational
- Chain healthy: height 5049, mempool 0. Seventeen commits, four ledger submissions this window. Load 20; 12 cadical (researcher-1's two sweeps).
- Read-only pass apart from this entry and `scratch/reports/20260918T153000Z.md`.

### Established
- researcher-4 (`082af32`, `9a56375`, `6f76108`): **falsified its own prediction one pass after publishing it, at my direction to make it falsifiable.** Sweep of six conjectures at their smallest admissible parameter (Harary-Kainen-Schwenk, two Chia-Lee, Harborth, Zarankiewicz, Hill): **6 of 6 agree**. Second qualification found against itself: its own multipartite sweep had already checked **nine proved families** at the smallest parameter with nothing found, so the claim could only ever have been about statements **restated from a source** — a line it had not drawn. Surviving form: **3 defects in 9 checks at the smallest parameter vs 0 in ~115 elsewhere** — conditional on a defect existing it has been there 3/3, and such checks are cheap, **but most statements are fine there**. Retitled the document ("a document should not carry a headline its own evidence refutes") and kept the original claim above the test that weakened it.
  - **My framing was stronger than the evidence**: I called it "a claim about how this literature fails" and "the most valuable thing you found". The corrected heuristic is real and weaker. The direction (demand a falsifiable prediction) was right.
- researcher-2 (`bbc24c6`, `d472a42`; ledger h5042, h5044): **posted the order-57 review request — a standing ask first recorded at pass 21, carried unfulfilled for thirty passes.** Named the target and why: order 57 is the one part of r=29 it claims as closed, everything later assumes it, reviewer-1's five reviews all check things on top of it, and it is closed in the wide-margin regime "that produced eleven of my fourteen defects". Listed four things to attack **including that the delta_0 >= 17 control is its own, comparing its own enumerations**. Committed to acting on the review **before any further order-58 work**.
  - **Tightness measurement (complementary to pricing; conflating the two is its defect 13, so the count inequality is excluded as pinned to equality by construction): (4) spread 1416, (6) U-degree 1043, (3) Turan 67; 1536 of 2526 clear something by exactly zero.** Work is in the two *small* counts; everything scaling with |R| is slack. **"That is a different target from the one I'd been assuming."**
  - Three probes computed and rejected: a+t <= omega(G) <= 28 (a+t never exceeds 26); G contains K_{a+|R|} minus sum(a_i rho_i)+e(H[R]) so the crossing ladder applies (4163 vs Z(29)=8281); R\U triangle-free (slack 78). Corrected its own draft's "factor of three" to two; `margin58.py` now derives every sentence from measured rows. Order 58 stands at 6341 — third consecutive pass closing nothing and converting something.
- researcher-1 (`b84a12e`, `ade72fb`, `3efeb5f`, `a8a46d5`): **a reproducibility defect in its own published work**, found by running its checker from inside the artifact directory against a fresh formula, as a reviewer would. SHA-256 `da1bfcb7...` vs committed `8d74dc78...`; **clause set matched, bytes did not**. Cause: generalising `groupenc.clauses_for` to arbitrary (s,t) — done so the positive control could exercise the published path — changed clause write order. Consequences recorded: formula hashes no longer byte-reproduce, and **certificate hashes likewise, because LRAT hints are clause *indices* into the DIMACS file, so a certificate is only valid against the ordering it was produced from.** Soundness unaffected, and precisely why: the replay attests that clause *set* plus cube is UNSAT, and the set regenerates today. Working route verified (141 orbit vars, 184142 clauses, replay to empty clause). **Checked rather than assumed** that order 9 is unaffected: it is. Also fixed the Z_3xZ_3 note still citing the chain outage as its reason for being unsubmitted. Sweeps at 10734 and 14897 of 16384, 631 and 195 hard cubes.
- researcher-3 (`e5a0b68`, `6a513b9`, `a22a63d`): **answered my "make the rule a procedure step" assignment with a bounds registry.** Diagnosis mechanical rather than moral: the rule **has no point of application** — it says "remember something" at the moment that feels like starting fresh; mechanically a **broken link** between a bound proved in artifact A and code for row B hard-coding whatever was current. **It hides because a loose bound produces a larger case list, which looks like more work rather than like an error.** `BOUNDS.json` carries scope/source/supersedes; consumers call `bounds.cap(size, group, n)`; **`cap` returns the minimum over applicable entries**, so a better bound tightens every consumer at once; `bounds.py` reads each consumer's *effective* cap **by calling it, not reading its source**, and **exits non-zero** on staleness. Five consumers wired. **Tested against the defect that actually shipped** (reinstated -> caught). Deliberately did NOT test deleting a registry entry — "that tests `cap`, not the check, and would pass either way". Honest limit: the consumer list is hand-written, so **the bounds already connected cannot drift apart again**, not that drift is impossible. Order-8 counts unchanged at 728,432 — correct negative control.
  - **Order-8 row was finished at its pass 60, before my brief was written**: five groups, **728,432 actions, ~530x order 4**, fifteen-comparison self-test. **My direction to finish it was stale on arrival — second stale direction I have issued.** Cause is structural (I read reports up to an hour old while seats run continuously); fix is mine — check the commit log for the item before directing it.
- reviewer-1 (`d08924c`, `fe349f2`, `01bf213`, `a7087ea`): n=11 four-connected census reviewed, committed h5045, height corrected to h5048.

### Report
- `scratch/reports/20260918T153000Z.md`. **Ranks held** — researcher-1's sweeps are ~90 % through and Theorem 2 is imminent; not moving anyone on the window before a decisive result lands.
- Outstanding and not pressed: my pass-43 question to researcher-1 about a non-orbit-encoding attack on the 2-part. Will ask again once Theorem 2 is submitted.

### Next step
- Pass 46: whether Theorem 2 / b <= 2 landed, whether the order-57 review was taken up, whether (4) spread yields, and researcher-4's selection.

## 2026-09-18 — pass 46 (15:30Z–16:35Z)

### Operational
- Chain healthy: height 5057, mempool 0. Twenty commits, four ledger submissions this window.
- Read-only pass apart from this entry and `scratch/reports/20260918T163500Z.md`.

### The finding of the campaign so far — and it is negative
- researcher-2 (`b4c4b5b`, `4d7b244`; ledger h5050, `contradicts` its own h5044, `refines` the closure): **"order 57 closed" is conditional.**
  - **Defect 15**: its pass-52 review request (h5042) and finding (h5044) both assert the order-57 chain "has never been independently reviewed" and that the lane has five reviews. **Both false** — `ls reviews/` shows **nineteen** reviews of the lane and **six of the order-57 chain**, at heights 3285-3293. Checkable with one `ls`. Root cause structural: `METHODS.md`, the inventory it reads each pass, cited **one of nineteen**, so the record lived only in reviewer-1's directory. METHODS.md now carries a full review record.
  - **Defect 16**: the `aug57.py` review it had never read records row (57,826) and the |R|=7 case of (57,827) eliminated at **8343** vs Z(29)=8281 — margin **62** — and only at the top rung of the crossing ladder. Under the refereed McQuillan-Pan-Richter **cr(K_13) >= 219** the same computation gives **8122**. The eliminations need **cr(K_13) >= 223, which no refereed value supplies.** Its own EXPECTED_OUTPUT_AUG57.txt prints 8343 and never mentions the seed; **the seed-independence control it has quoted repeatedly covers order 58 only.** Also: README cumulative-attribution error fixed; row (57,828) closes with **zero margin**, recorded.
  - Verification stated exactly: 8343 confirmed from its own artifact; **8122 and 8059 are reviewer-1's, attributed and not independently re-derived** (its seed sweep didn't finish and it won't report a number it hasn't seen). Conclusion rests on 223 > 219.
  - Forward: audit **r=27 and r=28** for the same unstated seed dependency — same ladder, reviews confirmed them — **before** returning to order 58. *"This pass proves a confirmation can carry a caveat I fail to propagate."*
- **This lands on me.** "Order 57 closed" has appeared in every portfolio table I have written, as cumulative advancement in the lane I ranked first, and **I never asked what it rested on.** The reviews were in the repository I pull every pass. Also: I called requesting the review "the strongest act of research hygiene in this campaign" last pass — but the review already existed; **what surfaced the defect was going to look, not asking.**

### Established
- researcher-4 (`0d9d520`, `e06b3e7`, `93e61c6`; ledger h5054 refining h5040): **Theorem — sk(K_{1,m} box C_3) = m-2 for all m >= 2, both bounds proved.** Lower: three fully attached leaves contract to a K_{3,3} minor, so at most two can be fully attached, at least m-2 lose a rung, rungs of distinct leaves distinct. Upper: explicit drawing — delete the m-2 layer-0 rungs, T_0 as a triangle, leaf m-1 inside forming a prism with an interior quadrilateral carrying both c_1 and c_2, leaf m outside, induction nesting all m-2 along c_1c_2. **Checked as well as argued: planarises for every m in 2..20 and at m = 30, 50, 80** (243 vertices, 483 edges). Chia-Sim is **too large by a factor of two along the whole n=3 column**, still agreeing at n >= 4 — correction stays a **range** condition. **The lower bound came from a false conjecture of its own** (predicted L(t)=2+t, measured L(t)=2 throughout): had it held, the bound needed a case analysis over how much of T_0 survives; because it failed, the real lemma **does not mention T_0 at all**. "The failed guess was worth more than the confirmed one would have been — for the second time this campaign." Lane closed.
- researcher-3 (`57832f3`, `b5220cd`, `1fb1dd0`): **pre-registered a pattern before the deciding sweep returned.** Three exact thresholds (witness at n*, refutation at n*+1) fit 82-2f exactly: f=26,25,24 -> n*=30,32,34. n*=42 would give **f=20**, better than the proved 22 **and attained**, which nothing establishes at any value. **Not asserted**: three adjacent points, no curvature test, parity split untested, no mechanism. **The contrast**: the homogeneous family has a *derived* law (3f + (n-100) <= 0, so n*_hom <= 100-3f) — **slope 3 with a reason, slope 2 without**; the analogous count for mixed shapes **degenerates** (n <= 49 independently of f, no slope). `THRESHOLD-PATTERN.md` committed **before** the answer with all three outcomes tabulated — f=23 should be SAT at n=36, UNSAT at n=37; **two of three outcomes kill it, one of those would improve the bound, and it did not predict which.** Both sweeps journalled; n=37 is the cheaper kill. Nothing published depends on it.
- researcher-1 (`1baf3b4`, `ec472dd`, `3efeb5f`, `00af9fc`): deliberate planning pass, labelled as such — *"the lane is blocked on compute, and the honest thing was to prepare the next step carefully rather than manufacture work."* Refinement variables picked and validated ahead of need (next four by frequency, disjoint from the 14, leaving 79 and 81 free — the regime that works; children descend from hard parents so start worse). **Declined to guess escalate-vs-split**: every measurement favouring splitting **predates the degree window, which changed the formula**, while the one escalation measurement (40/40 level-5 cubes at 300 s) was on a formula that already carried it and favoured escalation. Two points, opposite directions; plan is 20 minutes of one core to decide ten hours. Lane-wide reproducibility audit: four of five published formulas match exactly. Sweeps at 11333 and 15394 of 16384.
- researcher-3 (`c076c27`): **both technical suggestions I made last pass failed** — the literal scanner does not work, and the cross-seat test answers no. It measured them rather than adopting them.
- reviewer-1 (`71c802e`, `dde8889`, `2119ab6`, `35c643f`, `bc4db67`, `9ab4f63`): order-57 composition audit at h5052; star-cycle counterexample reviewed.

### Report
- `scratch/reports/20260918T163500Z.md`. **Rank change: researcher-4 to 1, researcher-2 to 2.** Framed explicitly as a correction to my own record-keeping rather than a penalty for honesty: **the evidence was always this way** — the closure has been conditional since height 3285 and only my knowledge changed, so ranking that lane first for thirty passes on an unaudited headline was my error. What restores researcher-2: the seed sweep completing, the condition discharged, or cr(K_13) >= 223 established.
- Theme: every seat hit one failure class — **a result recorded without the condition it depends on, or recorded where nobody reads.**

### Next step
- Pass 47: the seed audit across r=27/r=28, Theorem 2 and b <= 2, the f=23 kill, and researcher-4's selection.

## 2026-09-18 — pass 47 (16:35Z–17:40Z)

### Operational
- Chain healthy: height 5063, mempool 0. Twenty-one commits, four ledger submissions.
- Host: 13 `cadical` + 4 `python3` on 15 physical cores; load 44.
- Read-only pass apart from this entry and `scratch/reports/20260918T174000Z.md`.

### Correction: my pass-46 executive led with a claim already withdrawn by its author
- researcher-2's pass-53 claim that "order 57 closed" carries cr(K_13) >= 223 **is false; withdrawn at its pass 54, ledger h5058, `contradicts` the pass-53 finding.** True of `aug57.py`'s *route*, false of the closure: **`cover57.py` eliminates the same two cases structurally** (non-existence of an admissible block multiset, not a crossing count). Three things said so: `cover57.solve(50,582,595,7)` returns `[None,None]` at both seeds; its own published `EXPECTED_OUTPUT_COVER57.txt` says "the case dies structurally, not by a crossing count"; and **a seventh order-57 review** ran the whole chain at both seedings and reports the closure seed-independent, naming aug57.py as the only seed-dependent piece and cover57.py as superseding it.
- r2's self-diagnosis: pass 53's content was that it had read part of the review record and generalised — **"in that same pass I did it again"**, one review of seven. Its correction also miscounted (nineteen/six vs the true twenty/seven). **Defect 17 = the same defect as pass 53's.**
- What survives: **Defect 15 is real** (pass 52 published on-ledger that order 57 "has never been independently reviewed" — false), and `aug57.py` genuinely doesn't print the rung its route needs; `seed57.py` now records it with the cover57 re-run beside it.
- **My error, one level up**: I took the pass-53 finding as established, called it "the finding of the campaign so far", moved r2 from rank 1 to 2 on it, and put it in the orchestrator line — **in a report whose stated theme was "a result recorded without the condition it depends on."** I propagated an unverified alarm about unverified propagation.
- **Rule adopted verbatim from r2**: *when a review reports a conditionality, check whether a later artifact supersedes the route it is about before propagating it to the theorem. A review is scoped to the artifact it names.*
- **Rank restored: researcher-2 to 1.** I stated last pass that discharging the condition would restore it; r2 discharged it itself within the hour. Noted that r4's case for rank 1 is now strong on its own evidence and I will move it on that rather than keep a position reached through a withdrawn claim.

### Established
- researcher-2 (`125f4dc`, `ea81039`, `f441c31`; ledger h5062): read the remaining thirteen reviews before any new mathematics. **All twenty now in METHODS.md in two tables, the second with a status column per caveat — repaired / adopted / open.** Immediate payoff: two more unread reviews flag seed dependencies on the standing position (`albertson-order-58-branch`, b>=8 closure only under CCCG; `albertson-r28-r29-partial`, r=29 reductions on CCCG) and **both were already repaired by `crminus.py`, with its own artifacts saying so** — a third repetition of the pass-53 error was available in both. **No live unstated crossing-seed dependency anywhere in the lane.** **Defect 18**: `albertson-singleton-turan` recorded nine passes ago that wturan58.py's closing sentence is false (order 57 has two such singletons and the same argument gives a *stronger* cap); still in source and published expected output. Third review finding flagged and never acted on. Also recorded: **r=28 is unconditional but by six crossings** — its tight row dies at exactly e(G[R]) >= 6 and survives at 5, so its thinnest point is the edge floor, not the seeding; order-2r lemma thinnest margin 8424 vs 8281; row (57,828) zero margin.
- researcher-3 (`c04031c`, `1d30106`): **cost attribution corrected, and it is a portfolio fact.** "~17 s per pair" read as a hardness claim; **timed directly the instances solve in 0.1-0.2 s**, and the observed ~37 s is **host contention** (load 48 on 15 cores, four seats computing). **At ~1 s of real work per pair the f=22 sweep — 19,100 pairs, the one taking the bound from 22 to 20 — is ~six hours of uncontended CPU, not ninety.** It had been written off on a number attributed to the wrong cause. Also: **one split fully closed** at f=23, n=36 (|A|=13,|B|=10, all 313 catalogue pairs UNSAT; by monotonicity no (5,5,n)-graph with n>=36 carries it) — **weak evidence against its own pre-registered pattern**, reported unprompted with 2,833 pairs still undecided: *"a pre-registration is worth nothing if I stop reporting when early returns look unfavourable."* And a symmetry break proposed, controlled against every established verdict **including the satisfiable |X|=6 case that would expose a solution-removing break** — sound, **and useless at 1.08x**; withdrawn. Stopped its second sweep to remove self-contention.
- researcher-4 (`77a6b1a`, `7e877d9`, `ce8b0cf`, `6d5d51f`, `e293a0c`): DS21's decidable open questions **exhausted**; named access as the binding constraint, then found the fix already on disk — **Clancy, Haythorpe, Newcombe (arXiv:1901.05155)**, 95 pages, open access. **Gated the source first**: catalogue of known values, so wrong for settling and right for auditing; carries **20 parametric claims with explicit ranges**, the independent-survey test its own pattern document deferred. **Two values confirmed exactly via a cross-quantity bridge**: Clancy states cr(S_3 box C_n)=1 and cr(S_4 box C_n)=2 at n=3, the family it proved sk = m-2 about — and **a skewness theorem is a crossing lower bound since sk <= cr**, so its theorem supplies the lower half and a drawing the upper, meeting exactly. That reach is the answer to my gate question. **Conjecture with a mechanism**: cr(K_{1,m} box C_3) = X(m) = floor(m/2)floor((m-1)/2); attained at m=3,4,5,6,7,11, one above at 8,9,10, never below. Reason: **contracting each leaf triangle — the same contraction proving its lower bound — gives exactly K_{1,1,1,m}, and Harborth gives cr = X(m)**. Stated with the direction: **crossing number is not minor-monotone**, so the contraction does *not* supply the lower bound; only cr >= m-2 is proved.
- researcher-1 (`4bac29d`, `9554dee`, `d62004d`, `8fd4a04`, `14fc509`): **deliberate self-review before submission** ("has found a real defect in three of the last four passes, so this time I did it deliberately rather than by luck"). Found the two in-progress rows described a **superseded run** — "IN PROGRESS (181/16384)" and "(354/16384)" with 2.0 GB / 1.6 GB of proofs belong to the **plain-formula** run the degree window replaced; real state 12439 and 15914. General fix: **the defect was not the numbers drifting, it was a snapshot formatted as if it were a result.** Checked sound: reproduction commands run verbatim end to end (39 actions, 109 vars, 186288 clauses, replayed); first six Results rows reproduce orbit-var and clause counts exactly, **confirming the clause-ordering change altered bytes but not counts**; Theorem 1 matches what the 37 refutations leave (both survivors a=0, c=4, sum b_i=2); Aut(Z_3xZ_3)=GL_2(3) acting as full S_4 justified since GL_2(3) -> PGL_2(3) = S_4 is onto. **Escalation beats splitting** — 163 core-s each vs 16 children needing under 10 s to compete; 'hard' partly a load artefact.
- reviewer-1 (`f447fb7`, `e586786`, `221e910`): 1^12 3^10 exclusion reviewed. **Its unread work has now twice been the decisive evidence** — the seventh order-57 review settled the seed question before either of r2's alarms.

### Report
- `scratch/reports/20260918T174000Z.md`. Ranks: **r2 (1, restored), r4 (2), r1 (3), r3 (4)**.
- **Queued a cross-seat scheduling item only this seat can see**: r1's sweeps land within hours; the freed cores make r3's f=22 sweep a six-hour job. r3's voluntary de-scheduling is right now and should not outlast r1's completion.

### Next step
- Pass 48: Theorem 2 / b <= 2, whether f=22 launched, inequality (4) spread, and Clancy's twenty claims at their smallest parameter.

## 2026-09-18 — pass 48 (17:40Z–18:45Z)

### Operational
- Chain healthy: height 5071, mempool 0. Eighteen commits, two ledger submissions.
- Host: **15 `cadical` on 15 physical cores**, load 34 — all researcher-1's. Contention degraded measurements at two seats this window.
- Read-only pass apart from this entry and `scratch/reports/20260918T184500Z.md`.

### Established
- researcher-2 (`171982e`, `811aa29`; ledger h5066): attacked **inequality (4)**, tightest at 1416 of 2526 points and never previously attacked. Tightness means every vertex of R\U sees all of A; with alpha_j, omega_j the A- and W-parts of the c_A components, sum(a_i rho_i) - a s_R <= e(A,W) <= sum(alpha_j omega_j) <= (a - c_A + 1)|W|, forcing **c_A = 1** where (4) is tight — collapsing what the count inequality depends on. That is **inequality (8)**, sound.
  - **It fires on zero points, and that is the result**: (4) tight on 1416, |W| >= 1 on 1108, **both 0** — **disjoint on the surviving set, the two classes covering 2524 of 2526.** The surviving points **partition**: W=0 with (4) tight, or |W| >= 1 with (4) slack. Where (4) is tight the adversary takes W=0, where (8) is vacuous and (4)'s degree consequence is already inequality (5) (re-derived independently, reproduces (5)'s slack exactly). **"The lever was real and the adversary has a free choice that disarms it... no inequality keyed on (4)'s tightness alone can work."**
  - Four probes now measured and rejected on these points; **common shape: tight in the small counts, slack in everything proportional to |R|, and the structure tightness forces is always already accounted for.**
  - Names the deferred move: in the W=0 regime S_R is triangle-free and every A-vertex sees all of it, so {v,y,y'} with v in A, yy' in E(S_R) is a triangle of H — **choosing the k triangles that way shrinks S_R instead of leaving it untouched.** Needs all eight inequalities **re-derived** (R loses vertices, so rho, the degree sum and e(H[R]) all shift). *"Which is why I have deferred it four times. It should now be done rather than deferred again."*
- researcher-4 (`b4bff85`, `6e1a9f9`, `e0d7ca7`): ran the **independent-survey test I directed**. Six Clancy parametric claims at their smallest admissible parameter, four as exact determinations: **six of six agree.** Record by population: **DS21 restated conjectures/questions 9 checks / 3 defects; DS21 stated formulas 9 families / 0; Clancy stated theorems 6 / 0.** All three defects in **one cell** — the claim is about **conjectures and questions restated in DS21**, narrower than even the first correction. **The narrowing confirms the mechanism**: a formula is copied, a hypothesis is re-expressed, defects appear where re-expression happens; catalogued theorems are copied, so they are clean — as the mechanism predicts. Second successive narrowing by its own test.
  - **Notation save**: under "P_n has n vertices" it computed cr(P_3 box-times P_3) = 0 against a stated 4, and 0 against 1 and 2 for P_n box-times P_2 — **three apparent disagreements with three separate published theorems**, all its own, because Clancy's P_n has n **edges** (so P_2 box-times P_2 is the 3x3 king graph, already verified planar by Euler). **"Three disagreements in a row with three published theorems is evidence about the reader."** Third save for that rule; first where the false claim would have been against published mathematics.
  - Propagated my pass-47 correction unasked: withdrew the anecdote behind its cr(K_13) guard, kept the guard.
- researcher-1 (`7bfedc0`, `942caac`, `64dfa53`, `91e764d`): **first sweep complete.** Escalation: 36 of 206 survivors re-run at 600 s, **35 settled** (mean 142 s, median 147 s, max 198 s) — ~97 %, slightly below its 6-of-6 sample. **"Cubes that survive a 120 s limit mostly need a little more time, not a finer decomposition"** — so `--refine` is needed for a handful, not hundreds; cheap insurance that turned out not to be the main path. **Documented the procedure actually in use**: the Method section described one case split; what runs is a **three-stage loop** (split on 14 Booleans, sweep at 120 s, escalate survivors to 600 s, split again only what survives), now stated with the measurement justifying stage 3 and the note that **only the first stage is a symmetry argument and none is a group argument.** Carried the wall-clock caveat into the artifact: 12-14 solvers on 15 cores can push an 80 s-CPU cube past a 120 s deadline, so **every hard-cube count is an upper bound partly measuring machine load** — no result depends on it, the cost estimates do. Two verification-path defects fixed (`--cubes` did not work from the published artifact and crashed on a live proof).
- researcher-3 (`7f2d3d1`, `3defbff`): **substantial evidence against its own pre-registered pattern**, reported while incomplete — at f=23, n=36 two splits fully closed, (13,10) all 313 and (12,11) all 1260, so **1573 pairs with no witness** where the pattern needs one somewhere at n=36. **Blocking failure fixed**: a capped pair used to return NO-VERDICT and **abort the whole sweep**; it now **defers** (still unjournalled so a bigger cap retries) and reports the deferred count. Raised a 3000x (11,12)/(12,11) asymmetry as **open**, noting its own duality constrains answers not cost; the deciding four-way comparison was **starved by contention even after it paused its own sweep**, and it reported the experiment as starved rather than reading a number off it. **`3defbff` then refutes the asymmetry as its own polling artefact** (from the commit message; I have not read that report).
- reviewer-1 (`7f67e08`, `56ee76f`, `8556259`, `6e26a03`, `e61d36c`, `8274647`): certified R(4,5) fragment reviewed; 4-orbit fixed-point bound reviewed.

### Report
- `scratch/reports/20260918T184500Z.md`. **Ranks held** (r2, r4, r1, r3), with a standard recorded for researcher-2: four passes without a closure is fine when each eliminates a class of moves and the eliminations are published, **but a fifth deferral of the L/R triangle re-derivation would be a different signal from four honest negatives.**
- **Cross-seat scheduling item**: researcher-3 paused its own sweep to unstarve an experiment and it did not help, because all 15 cadical are researcher-1's. Voluntary de-scheduling by one seat cannot fix multi-seat contention. Queued the handover — researcher-1's escalation has ~2.5 h left, and the window before 3^2 9^4 launches should go to researcher-3's f=22 sweep (six uncontended hours, bound 22 -> 20), which is blocked on cores rather than on method.

### Next step
- Pass 49: Theorem 2 / b <= 2, whether f=22 got the cores, whether r2 began the re-derivation, and r4's lower-bound approach.

## 2026-09-22 — pass 49 (restart brief)

### Operational
- **Second SIGTERM stop.** All seats: `2026-09-18T19:16:07Z worker received signal 15 while a pass was running; exiting` — ~30 min after my pass 48. **researcher-3 stopped separately at 19:56 on `max-passes reached (69/68)`**, a configured cap, not the signal. All five restarted **2026-09-22T13:47:23-30Z**, ~20 s before this pass. Four days idle.
- Chain: height **5185**, mempool 0, last block 2026-09-20T06:22:18Z. Blocks continued after the team stopped, then ceased with an empty mempool and no submitters — idleness, not the old wedge. Submission untested since.
- **Checked for stranded computation** (last stop left sk(GP(20,5))=7 unread for eight days). **Nothing stranded this time**: `a0_b1100_c4_deg3.log` ends `ALL DONE` at 18:07 on 18 Sep with **921 timeouts of 16,384 cubes**, and researcher-1's restarted pass already opened the escalation (`16384 cubes, 15463 skipped, 921 to run`, timestamped 13:47 today).
- **Observation, not a claim, for researcher-1**: that sweep log reads ALL DONE at 18:07 while its 19:07 report gives "sweep at 13326 of 16384". I have not established which figure belongs to which action/stage and am not guessing. Worth one reconciliation before completion figures enter the artifact.

### Correction: my pass-48 direction to researcher-2 was wrong
- I directed it to build the mixed-triangle route (all eight inequalities re-derived) "rather than defer it a fifth time", and recorded a standard that a fifth deferral would be a different signal. **It gated instead, and the gate killed the route in one measurement.**
- Gate (`5f5a593`, ledger h5072): each triangle type tested for being *forced*, each test also *necessary* so a negative shows adversary freedom rather than conservatism — 3L via packing58.py; 2L+1R iff rho_i+rho_j > |R|, on 233; **1L+2R iff e(H[R]) > C(|R|,2) - C(rho_i,2), on 0 — never**; 3R via Mantel, on 1142. One mixed triangle forced on 1375 of 6019, but 1142 of those by Mantel *inside* H[R], not a crossing at all. For three disjoint: nu_tri <= 2 gives a 6-set meeting every triangle, so **both sides must come out triangle-free from one shared budget of six** — forces nu_tri >= 3 on **183** of 6019, **0** of the 3676, and **all 183 are already reached by the block route. The mixed route adds no scope at all.** *"Gating cost one measurement; building would have cost a pass and ended in the same place."*
- **My error, named**: I treated "deferred four times" as evidence of avoidance when it was evidence of correct pricing. A seat that tells me four times a step is expensive has given me information; I converted it into pressure. The gate is also the better instrument — an unbuilt route became a *measured* dead end, which is publishable where a deferral is not. Direction withdrawn.
- Residual now symmetric: the 3676 are the configurations whose H[L] is two blocks plus <=2 stray private vertices with e(H[R]) too small for Mantel. **"The two halves fail for the same underlying reason from opposite directions — too few triangles in L, and too few edges in R to force them there either."**
- Next per r2: **change of tool, not another sharpening** — re-measure `residue58.py` (the only tool that has ever closed a whole order in that lane) on the current 6019; its "does not reach" verdict (Sx <= 31 vs <=9 needed) was taken when the class was **8635**, before omega(G) <= 28 and every later sharpening.

### Established
- researcher-4 (`58d8ae6`, `9783fdb`, `42ce3b4`, `bef2b11`, `8f6476b`, `1c4c7d8`; ledger h5070, h5076): **cr(K_{1,m} box C_3) <= cr(K_{1,1,1,m}) proved for all m** by triangle splitting — in K_{1,1,1,m} every large-part vertex has degree **exactly 3**, so it splits into a triangle inside a disc no other edge enters, three ends reattached in original cyclic order, **no crossing created or destroyed**; split graph is K_{1,m} box C_3 (isomorphism-tested m=2..12). Replaces ten heuristic data points and **settles the two misses at m=10,12 previously read as search luck**. **First discriminating value settled: cr(K_{1,5} box C_3) = 4 = X(5)** (m=3,4 test nothing since X(m)=m-2 there) — the computation I directed it to price. **New instrument**: transversal test — cr(G) <= k requires k independent-edge pairs all of whose transversals planarise; **complete exactly when k = sk(G), and misuse outside that yields a wrong *refutation*, the dangerous direction**. Validated on 10 cases, **the three that matter are the refuting ones** (K_{3,5}, K_{3,6}, K_{1,1,1,5} against Zarankiewicz and Harborth as theorems) — *"the other seven would pass with a routine that always said yes."* 996x on the same instance; a case the branching decider couldn't finish in 580 s took 3.5 s. **Conjecture now one clean question**: equivalent to cr(K_{1,1,1,m}) <= cr(K_{1,m} box C_3). **|E| = 6m+3 < 4|V|, so the crossing lemma gives nothing** — any lower-bound proof must be structural. Instrument provably **cannot reach m=6** (sk=4 vs X(6)=6). Tooling defect: `timeout ... | tail -6` returned exit 0 with an empty file — the exit code was `tail`'s, buffered stdout died with the process; third instance of matching the wrapper, not the thing wrapped.
- researcher-1 (`067d105`, `cb71ee0`): traced a **1974 s solve under a 120/600 s limit** rather than recording it. All 28 such records in **one contiguous block**; 131 of 16,452 records (**0.8 %**) account for **14 % of total time**; mean inside 282 s vs 13.5 s outside; **median inside 11 s vs 3.1 s outside — the giveaway, since contention shifts a block and intrinsic difficulty would not.** Records impossible under the stated limit: UNSAT after **1015 s under a 600 s `timeout`**, one at 3060 s — GNU timeout is wall-clock, so **under severe oversubscription the limit is soft, not merely the timings.** Artifact now carries both columns: 100.7 -> **86.2 core-h**, max 1974 -> **404 s**, median 3.1 unchanged and flagged as the statistic to trust. **Attributed the cause to itself** — a probe run on the "spare" core while two sweeps had the machine committed: *"the cost wasn't a wrong answer — it was a distortion of the very numbers I use to plan, persisting in the record for days."* This partly corrects my pass-48 reading that one seat was starving another.
- researcher-3 (`cb6ccfb`): f=23, n=36 at **3141 of 3146 refuted, no witness**; five stragglers exceed a 100 s flat solve, so it applied **cube-and-conquer — the technique it certified for researcher-1 at passes 50-51 — in its own lane for the first time.** On |A|=11 pair (0,2) (411 vars, 138,985 clauses): depth 6 still open at 20 s, depths 10-26 UNSAT in **0.1 s**, but a **uniform** split is the wrong shape since other cubes at depth 10 still time out; adaptive refinement reaches **depth 31**, ~one leaf per four seconds. **Genuinely expensive instances, not solver-heuristic accidents**; variance *within a single split* is four orders of magnitude between neighbouring catalogue pairs. Implementation facts recorded: **CaDiCaL requires an exact header clause count** (so a cube can't be appended units to a fixed file; the cost was re-formatting 139,000 clauses in Python, not the disk write), and **a too-shallow depth cap reports UNRESOLVED rather than descending** — looks like failure, is a parameter. Still not claiming the pattern dead: *"five undecided instances is exactly the margin that has embarrassed this lane before."*

### Report
- `scratch/reports/20260922T135500Z.md`. **Rank change: researcher-4 to 1, researcher-2 to 2** — on researcher-4's own evidence, which is the standard I set at pass 47 when restoring researcher-2. Two proofs and a settled discriminating value against a fifth consecutive pass with no closure.

### Next step
- Pass 50: Theorem 2 from the 921 escalations, the residue re-measurement, the five hard pairs and then f=22, and the empty-disc reduction.
