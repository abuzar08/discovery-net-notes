...
Last pass I reported that the \((11,12)\) block at \(n = 36\) was
**qualitatively harder** — *"every pair tried since has hit the 300 s cap"*, a
factor of at least \(3000\) — and speculated it was a shape effect my
complementation duality would explain. I flagged the deciding test as starved
and left the question open.

**The claim was wrong.** Timing the exact pairs in sweep order:

| pair | verdict | seconds |
|---|---|---|
| \((0,0)\), \((0,1)\) | UNSAT | 0.1 each |
| **\((0,2)\)** | **no verdict** | **> 60** |
| \((0,3)\) … \((1,1)\) | UNSAT | 0.1 each |

**One** instance is hard; its neighbours take a tenth of a second. What happened
is that I killed and relaunched the sweep repeatedly while polling it, and each
relaunch replayed the journal, arrived at \((0,2)\), and burned its time there
before I killed it again. The journal never advanced past \(1575\) and I read
that as the block being hard.
...
was running the work"* — and I made it again three passes later in the mirror
...
Hard instances appear in all four at about the same rate. **Hardness is
per-instance, not per-block or per-shape**, so there is no asymmetry to
explain. The "open question" was a bad question generated by a bad measurement.

The deferral I added last pass is the right fix and remains right — a capped
pair is skipped and the sweep goes on — but it cannot help if I kill the
process before the cap elapses. Sweep relaunched with a **120 s** cap, which
given that hard pairs are isolated should carry it through the rest of
\((11,12)\) and \((10,13)\) quickly.
