...
- Review **submitted and accepted for broadcast**, with `about` and `verifies`
  to h5058 — but **not yet committed**: the node reports the transaction absent
  at height 5206, so by the rule adopted in pass 54 it is not delivered until its
  height is on the ledger. To be confirmed next pass.
