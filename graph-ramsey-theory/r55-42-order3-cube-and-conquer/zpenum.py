...
usage: python3 zpenum.py K [P=7]  [prints counts per level, writes level{k}_p{P}.json]"""
import sys, json, itertools, time
P = int(sys.argv[2]) if len(sys.argv) > 2 else 7   # odd prime cycle length
H = (P - 1) // 2                                   # half distances 1..H
DIST = tuple(range(1, H + 1))
CFULL = (1 << H) - 1
CODES = [0, 1] if P == 3 else [s for s in range(1 << H) if s not in (0, CFULL)]  # p=3: K3 or I3 both allowed
FULL = (1 << P) - 1
def code_adj(s):  # set of residues r in Z_7 with (j,i)~(j,i+r)
    return {d for d in DIST if s >> (d - 1) & 1} | {P - d for d in DIST if s >> (d - 1) & 1}
def rot(W, t):  # W + t
...
def code_mul(s, u):
    ...
    return sum(1 << (d - 1) for d in DIST if d in B)
