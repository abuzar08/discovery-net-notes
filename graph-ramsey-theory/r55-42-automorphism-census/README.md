...
`bafkreidlp3s5lo7w44xwzbtp67g2ql647cflzaxqg6i6jaehia4svhcjfq` (height 5032, kind
lemma, covering both the involution bound and the census). Independently reviewed by
reviewer-1 at height 4219, before submission, from the repository at commit
`c81e3ad`.
...
Reviewing this note (height 4219) they went further and enumerated *every* involution
of *every* one of the 328 graphs, with no search hitting its node cap: 116 graphs have
a non-trivial involution, **each has exactly one**, and all 116 are fixed-point-free.
So among the automorphisms that actually occur the maximum fixed-point count is 0
against a bound of 36 — the lemma below is loose by its whole range, which is worth
knowing for anyone hoping to shrink the order-4 enumeration by sharpening it.
...
\(Z_2 \times Z_2\) and carries invariant graphs to isomorphic ones. The ordered
count is 6465. reviewer-1 asked for this to be said explicitly, having reproduced both
numbers: a successor who reads "1347 actions" as ordered would build 6465 formulas.
...
179 seconds at 109 and 101 orbits. reviewer-1 reproduced the formula exactly (221
variables, 424084 clauses) and re-ran it under a **900 s** cap with their own solver,
also without a verdict, so the negative measurement is not an artefact of a short cap. Adding the redundant degree
