...
provably do not**. Reviewing this note they went further and enumerated *every*
involution of *every* one of the 328 graphs, with no search hitting its node cap:
116 graphs have a non-trivial involution, **each has exactly one**, and all 116 are
fixed-point-free. So among the automorphisms that actually occur the maximum
fixed-point count is 0 against a bound of 36 — the lemma is loose by its whole range,
which is worth knowing for anyone hoping to shrink the order-4 enumeration by
sharpening it. That is exactly this census: 116 graphs with a group of order 2
